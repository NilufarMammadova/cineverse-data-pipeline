...
# exactly the AZAL pattern. Cleaning is dispatched per dataset.
# ============================================================
import logging
 
from cineverse.common import get_spark, jdbc_params, get_pg, audit
from cineverse.cleaning import CLEANERS
...
def load_to_silver(dataset, bronze_schema, bronze_table, silver_schema, silver_table, **_):
    spark = None
    try:
        spark = get_spark(f"CineVerse_Silver_{dataset}")
        ...
        # column types that match the cleaned DataFrame (avoids date/text mismatch
